...
export class DApp extends React.Component {
    ...
    initialize(userAddress) {
        // 1.保存用户的地址
        this.state.currentAddress = userAddress;
    ...
    async createContractObj() {
        ...
        console.log(address);
        // 通过abi文件和合约地址创建合约对象
        this.contractObj = new ethers.Contract(contractAddress.Token, contractABI.abi, singer);
    }
    ...
    async getUserAccountInfo() {
        // 获取用户的以太币
        this.state.balance = await this.provider.getBalance(this.state.currentAddress);
        // 获取用户的nft数量
        this.state.TripNFT = await this.contractObj.balanceOf(this.state.currentAddress);
    }
