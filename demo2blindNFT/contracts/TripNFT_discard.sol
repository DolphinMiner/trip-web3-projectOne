...
contract TripNFT_discard is Ownable, ERC721Enumerable {
    ...
    // 白名单merkleTree的根hash
    bytes32 public merkleRootHash = 0x78019a8a257e21d37983e2495f66b3115b8884916ea789c3d1228eeefd1f4620;
    ...
    // 元数据的baseURI
    string public baseURI = "ipfs://QmTKySihsR9BRUzsfWXKt3LeMCb1BSxpLQjndJLhiEe8pk";

    // 特殊URI
    string public specialURI = "";

    // 盲盒封面的元数据
    string public unrevealedURI = "ipfs://QmRzUk7SgUgU928GkYFe5yerLQzVke1AVx9jhimAziBcAU";
     ...
     */
    function isValidUser(bytes32[] calldata merkleProof) public view returns (bool) {
        // 生成当前要校验节点的hash值
     ...
     * @param merkleProof: 合约调用账户的 merkleProof
     */
    function preMint(uint256 numberOfTokens, bytes32[] calldata merkleProof) public payable {

        // 校验msg.sender是否是白名单用户
        require(isValidUser(merkleProof) == true, "msg.sender is not in whitelist");

        // 校验当前合约账户中的nft数量 + 铸造的数量 <= nft的最大供应数量
        require(totalSupply() + numberOfTokens <= MAX_SUPPLY, "Sale would exceed max supply");

        // 校验nft是否开始售卖
        require(saleIsActive, "Sale must be active to mint NicMetas");

        // 校验msg.sender所拥有的的nft数量需要小于每个账户nft的限制个数
        require(balanceOf(msg.sender) + numberOfTokens <= maxBalance, "Sale would exceed max balance");
    ...
    function publicMint(uint256 numberOfTokens) public payable {
        ...
        require(numberOfTokens.mul(mintPrice) <= msg.value, "Not enough ether sent");
        // 校验当前合约账户中的nft数量 + 铸造的数量 <= nft的最大供应数量
        require(totalSupply() + numberOfTokens <= MAX_SUPPLY, "Sale would exceed max supply");
        ...
        // 校验msg.sender所拥有的的nft数量需要小于每个账户nft的限制个数
        require(numberOfTokens + balanceOf(msg.sender) <= maxBalance, "Sale would exceed max balance");
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory)
    {
        ...
        if (bytes(URI).length > 0) {
            return string(abi.encodePacked(base, '/', URI, '/', tokenId.toString(), baseExtension));
        }
